...
function RippleButton({  children,  style,  onClick,  disabled,  className,  type,}: {  children: React.ReactNode;  style?: React.CSSProperties;  onClick?: () => void;  disabled?: boolean;  className?: string;  type?: "button" | "submit" | "reset";}) {  const [ripples, setRipples] = useState<RippleState[]>([]);
  const addRipple = (e: React.MouseEvent<HTMLElement>) => {    const rect = e.currentTarget.getBoundingClientRect();    const id = Date.now() + Math.random();    setRipples((r) => [...r, { x: e.clientX - rect.left, y: e.clientY - rect.top, id }]);
    setTimeout(() => setRipples((r) => r.filter((p) => p.id !== id)), 650);
  };
  return (    <button      type={type}      className={className}      disabled={disabled}      onClick={onClick}      onMouseEnter={addRipple}      onPointerDown={addRipple}      style={{ position: "relative", overflow: "hidden", ...style }}    >      {ripples.map((r) => (        <span          key={r.id}          onAnimationEnd={(e) => {            e.stopPropagation();            setRipples((prev) => prev.filter((p) => p.id !== r.id));          }}          style={{            position: "absolute",            left: r.x - 12,            top: r.y - 12,            width: 24,            height: 24,            borderRadius: "50%",            background: "rgba(255,255,255,0.35)",            pointerEvents: "none",            transform: "scale(0)",            animation: "btn-ripple 0.6s ease-out forwards",            zIndex: 1,          }}        />      ))}      <span style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", gap: "inherit" }}>{children}</span>    </button>  );
}
        ...
        {loading || response ? (
          <motion.div key="result" className="flex flex-col" style={{ width: "100%", padding: "80px 16px calc(18px + env(safe-area-inset-bottom))", minHeight: 0, position: "absolute", left: 0, right: 0, top: 0, bottom: 0 }} initial={{ opacity: 0, scale: 0.92, y: 28 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 260, damping: 26 }}>
            ...
            <div style={{ marginTop: 24, flexShrink: 0, height: 52, boxSizing: "border-box", borderRadius: 12, border: isDark ? "0.8px solid #312f2f" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#161616" : "#ffffff", display: "flex", alignItems: "center", padding: "0 12px", overflow: "hidden" }}>
              ...
              />
              <RippleButton onClick={() => onSend()} disabled={loading} style={{ display: "flex", alignItems: "center", borderRadius: 6, backgroundColor: "#5b5bff", padding: "6px 8px", cursor: "pointer", opacity: loading ? 0.6 : 1 }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              ...
              width: compact ? 425 : 574,
               ...
               transition: "background-color 0.4s ease, border-color 0.4s ease",
                  ...
                  letterSpacing: compact ? "-0.9px" : "-1.2px",                  color: isDark ? "#ffffff" : "#222",                }}              >                Welcome, Explorer              </h1>            </motion.div>            <motion.div variants={cardItemVariants}>              <div className="absolute" style={{ left: compact ? 16 : 54, top: compact ? 73 : 75, width: compact ? 393 : 467, height: 52 }}>                <UrlInputBar compact={compact} activeTheme={activeTheme} method={method} setMethod={setMethod} url={url} setUrl={setUrl} loading={loading} onSend={onSend} />              </div>            </motion.div>            {!headerExpanded && (              <motion.div                variants={cardItemVariants}                className="absolute flex items-center"                style={{ left: compact ? 148 : 222, top: compact ? 141 : 143, gap: 16, cursor: "pointer" }}                onClick={() => setHeaderExpanded(true)}              >                <div className="flex items-center" style={{ gap: 4 }}>          <svg width="16" height="16" viewBox="0 0 25 25" fill="none">            <path fillRule="evenodd" clipRule="evenodd" d="M12.0369 8.71262C12.4511 8.71262 12.7869 9.0484 12.7869 9.46262V16.611C12.7869 17.0253 12.4511 17.361 12.0369 17.361C11.6227 17.361 11.2869 17.0253 11.2869 16.611V9.46262C11.2869 9.0484 11.6227 8.71262 12.0369 8.71262Z" fill="#5B5BFF" />            <path fillRule="evenodd" clipRule="evenodd" d="M7.70886 13.0368C7.70886 12.6226 8.04465 12.2868 8.45886 12.2868H15.6147C16.0289 12.2868 16.3647 12.6226 16.3647 13.0368C16.3647 13.451 16.0289 13.7868 15.6147 13.7868H8.45886C8.04465 13.7868 7.70886 13.451 7.70886 13.0368Z" fill="#5B5BFF" />            <path fillRule="evenodd" clipRule="evenodd" d="M4.96051 5.96045C3.66147 7.25949 3.05005 9.42738 3.05005 13.0368C3.05005 16.6463 3.66147 18.8142 4.96051 20.1132C6.25956 21.4123 8.42744 22.0237 12.0369 22.0237C15.6463 22.0237 17.8142 21.4123 19.1133 20.1132C20.4123 18.8142 21.0237 16.6463 21.0237 13.0368C21.0237 9.42738 20.4123 7.25949 19.1133 5.96045C17.8142 4.6614 15.6463 4.04999 12.0369 4.04999C8.42744 4.04999 6.25956 4.6614 4.96051 5.96045ZM3.89985 4.89979C5.6437 3.15594 8.34424 2.54999 12.0369 2.54999C15.7295 2.54999 18.4301 3.15594 20.1739 4.89979C21.9178 6.64364 22.5237 9.34418 22.5237 13.0368C22.5237 16.7295 21.9178 19.43 20.1739 21.1739C18.4301 22.9177 15.7295 23.5237 12.0369 23.5237C8.34424 23.5237 5.6437 22.9177 3.89985 21.1739C2.156 19.43 1.55005 16.7295 1.55005 13.0368C1.55005 9.34418 2.156 6.64364 3.89985 4.89979Z" fill="#5B5BFF" />          </svg>                  <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: 14, letterSpacing: "-0.56px", color: "#5b5bff" }}>                    Header                  </span>                </div>                <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: 14, letterSpacing: "-0.56px", color: "#9e9e9e" }}>                  Optional                </span>              </motion.div>            )}            </motion.div>            <AnimatePresence>              {headerExpanded && (<>                <motion.div                  key="headers-panel"                  className="absolute"                  style={{                    left: compact ? 16 : 54,
                    ...
                    overflow: "hidden",                  }}                  initial={{ height: 0, opacity: 0 }}                  animate={{ height: "auto", opacity: 1 }}                  exit={{ height: 0, opacity: 0 }}                  transition={headersPanelTransition}                >                  <div                    style={{                      borderRadius: 6,                                            backgroundColor: isDark ? "#0f0f0f" : "#fcfcfc",
...
function BrowseApiSection({ compact, w1440, isDark, onBack }: { compact?: boolean; w1440?: boolean; isDark: boolean; onBack: () => void }) {
  ...
  const apis: { name: string; desc: string; color: string; letter: string; category: string; logo?: string; logoColor?: string; keyDesc?: string; cardKeyDesc?: string; tag?: string; keyLabel?: string; cardBg?: string; panelBg?: string; panelBorder?: string; labelColor?: string; height?: number; url?: string; auth?: string; auth2?: string; authNote?: string; getAKeyUrl?: string }[] = [
    ...
    { name: "Paystack", desc: "initialize transactions for your next product", keyDesc: "Requires a secret test key from your Paystack dashboard", color: "#72D46B", letter: "P", category: "Payment", logo: "/api-logos/paystack.png", url: "https://api.paystack.co/transaction", auth: "Authorization: Bearer your_secret_key", getAKeyUrl: "https://dashboard.paystack.com/#/settings/developer" },
    { name: "Clerk", desc: "Explore users, sessions, and identity data", keyDesc: "Requires a secret key from your clerk instance", color: "#6C47FF", letter: "C", category: "Auth", logo: "/api-logos/clerk.png", url: "https://api.clerk.com/v1/users", auth: "Authorization: Bearer your_secret_key", getAKeyUrl: "https://dashboard.clerk.com/apps" },
    { name: "WorkOS", desc: "Test directory sync and enterprise sign-in", keyDesc: "Requires an API key from your WorkOS dashboard", color: "#4A6CF7", letter: "W", category: "Auth", logo: "/api-logos/workos.png", url: "https://api.workos.com/directories", auth: "Authorization: Bearer your_secret_key", getAKeyUrl: "https://dashboard.workos.com/api-keys" },
    { name: "OpenAI", desc: "Send your first prompt to a language model", keyDesc: "Requires an API key from platform.openai.com", color: "#10A37F", letter: "O", category: "AI", logo: "/api-logos/openai.png", url: "https://api.openai.com/v1/models", auth: "Authorization: Bearer your_api_key", getAKeyUrl: "https://platform.openai.com/api-keys" },
    { name: "Anthropic", desc: "check available Claude models and version", keyDesc: "Requires an API key from console.anthropic.com", color: "#D97757", letter: "A", category: "AI", logo: "/api-logos/anthropic.png", url: "https://api.anthropic.com/v1/models", auth: "x-api-key", auth2: "anthropic-version: 2023-06-01", getAKeyUrl: "https://console.anthropic.com/settings/keys" },
    { name: "Replicate", desc: "Browse models ready for your next experiment", keyDesc: "Requires an API key from your replicate account", color: "#FF6B6B", letter: "R", category: "AI", logo: "/api-logos/replicate.png", url: "https://api.replicate.com/v1/models", auth: "Authorization: Token your_api_token", getAKeyUrl: "https://replicate.com/account/api-tokens" },
    { name: "OpenWeather", desc: "Get the current weather for any city", keyDesc: "Requires a free API key from Openweather. Added to the URL , not a header", cardKeyDesc: "requires a free API key from openweather", color: "#F48120", letter: "W", category: "Data", logo: "/api-logos/openweather.png", url: "https://api.openweathermap.org/data/2.5/weather?q=London", auth: "API key", authNote: "Appended to the request as &appid=your_key, never sent as a header", getAKeyUrl: "https://home.openweathermap.org/api_keys" },
  ...
  return (
    ...
    >
      ...
      <div style={{ borderRadius: 6, backgroundColor: isDark ? "#0C0C0C" : "#ffffff", padding: "16px 16px 16px", flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div className="hide-scrollbar" style={{ display: "flex", flexDirection: "column" }}>
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, ease: "easeOut" }} style={{ position: "sticky", top: 0, zIndex: 2, backgroundColor: isDark ? "#0C0C0C" : "#ffffff", paddingBottom: 16 }}>
          ...
          {filters.map(f => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 500,
                color: activeFilter === f ? (isDark ? "#F7F7F7" : "#656363") : (isDark ? "#9e9e9e" : "#818181"),
                backgroundColor: activeFilter === f ? (isDark ? "#2F2F2F" : "#efefef") : (isDark ? "transparent" : "#fcfcfc"),
                borderRadius: 4,
                border: "none",
                padding: "4px 10px",
                cursor: "pointer",
        ...
        <div style={{ position: "relative", borderRadius: 6, backgroundColor: isDark ? "#0C0C0C" : "#ffffff", border: isDark ? "0.8px solid #2d2d2d" : "0.8px solid #f2f2f2", padding: isDark ? "12px 16px" : 16, flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div ref={scrollRef} onScroll={handleScroll} className="hide-scrollbar" style={{ position: "relative", width: "100%", flex: 1, minHeight: 0, overflowY: "auto", display: "flex", flexDirection: "column", overscrollBehavior: "contain" }}>
        <motion.div
          key={activeFilter}
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div key={api.name} onMouseEnter={() => setHoveredApi(api.name)} onMouseLeave={() => setHoveredApi(null)} style={{ borderRadius: 4, backgroundColor: isDark ? (hoveredApi === api.name ? "#242424" : "#141414") : (api.cardBg ?? (hoveredApi === api.name ? "#fcfcfc" : "#ffffff")), border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", padding: 12, display: "flex", flexDirection: "column", ...((compact || w1440) ? {} : { height: api.height ?? 205 }), transition: "background-color 0.15s ease", cursor: "pointer" }} onClick={() => setSelectedApi(api)}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: w1440 ? 12 : 14, letterSpacing: w1440 ? "-0.6px" : "-0.7px", color: isDark ? "#E5E5E5" : "#656363", cursor: "pointer" }}>Get example</span>
                <div style={{ borderRadius: 4, backgroundColor: isDark ? "#232323" : "#efefef", padding: "4px 6px" }}>
                  <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: 12, fontWeight: 500, color: isDark ? "#959595" : "#656363" }}>{api.tag ?? "API Key"}</span>
                </div>
              </div>
            </div>
            <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 14 : 16, fontWeight: 600, letterSpacing: compact || w1440 ? "-0.7px" : "-0.8px", color: isDark ? "#FFFFFF" : "#585858", marginTop: w1440 ? 10 : 11 }}>{api.name}</span>
            <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 12 : 14, letterSpacing: compact || w1440 ? "-0.48px" : "-0.56px", color: isDark ? "#F7F7F7" : "#939393", lineHeight: "20px", marginTop: w1440 ? 4 : 3 }}>{api.desc}</span>
            {api.keyLabel ? (
              <div style={{ borderRadius: 4, backgroundColor: isDark ? "#2E1800" : (api.panelBg ?? "#fff4e8"), border: isDark ? "1px solid #934E00" : (api.panelBorder ?? "0.8px solid #FFE8CC"), width: 112, height: 26, padding: "0 6px", display: "flex", alignItems: "center", gap: 8, marginTop: (compact || w1440) ? 8 : "auto" }}>
              ...
              <div style={{ borderRadius: 4, backgroundColor: isDark ? "#2E1800" : (api.panelBg ?? "#fff4e8"), border: isDark ? "1px solid #934E00" : (api.panelBorder ?? "0.8px solid #FFE8CC"), padding: 8, display: "flex", flexDirection: "column", gap: 4, marginTop: (compact || w1440) ? 8 : "auto" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {api.keyDesc ? (() => {
                  ...
                  const m = text.match(/(platform\.openai\.com|openweathermap\.org|console\.anthropic\.com)/);
                  const break1440 = w1440 && (api.name === "Stripe" || api.name === "Clerk");
                  if (!m) return <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 12 : 14, letterSpacing: compact || w1440 ? "-0.48px" : "-0.56px", color: "#DC7800", lineHeight: "20px", maxWidth: break1440 ? 170 : undefined }}>{text}</span>;
                  const url = m[0], before = text.slice(0, m.index!), after = text.slice(m.index! + url.length);
                  return <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 12 : 14, letterSpacing: compact || w1440 ? "-0.48px" : "-0.56px", color: "#DC7800", lineHeight: "20px", maxWidth: break1440 ? 170 : undefined }}>{before}<a href={`https://${url}`} target="_blank" rel="noopener noreferrer" style={{ color: "#DC7800", textDecoration: "none" }} onMouseEnter={e => { e.currentTarget.style.textDecoration = "underline"; }} onMouseLeave={e => { e.currentTarget.style.textDecoration = "none"; }}>{url}</a>{after}</span>;
                })() : null}
              </div>
            )}
          </div>
        ...
        <AnimatePresence>
          {showTopBlur && (
            <motion.div
              ...
              style={{ position: "absolute", top: 0, left: 0, right: 0, height: 32, zIndex: 10, pointerEvents: "none", borderRadius: "8px 8px 0 0", background: isDark ? "linear-gradient(to bottom, rgba(12, 12, 12, 0.85), rgba(12, 12, 12, 0))" : "linear-gradient(to bottom, rgba(255, 255, 255, 0.85), rgba(255, 255, 255, 0))", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", maskImage: "linear-gradient(to bottom, black 20%, transparent 100%)", WebkitMaskImage: "linear-gradient(to bottom, black 20%, transparent 100%)" }}
            />
          )}
        </AnimatePresence>
        <AnimatePresence>
          {showBottomBlur && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.18 }}
              style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 32, zIndex: 10, pointerEvents: "none", borderRadius: "0 0 8px 8px", background: isDark ? "linear-gradient(to top, rgba(12, 12, 12, 0.85), rgba(12, 12, 12, 0))" : "linear-gradient(to top, rgba(255, 255, 255, 0.85), rgba(255, 255, 255, 0))", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", maskImage: "linear-gradient(to top, black 20%, transparent 100%)", WebkitMaskImage: "linear-gradient(to top, black 20%, transparent 100%)" }}
            />
          )}
        </AnimatePresence>
        </div>
      ...
      <AnimatePresence>
        {selectedApi && (
          ...
          <motion.div
            ...
            style={{ position: "fixed", left: "50%", top: "50%", width: 394, height: selectedApi.name === "OpenWeather" ? (detailBig ? 445 : 443) : selectedApi.name === "Paystack" ? (detailBig ? 403 : 391) : selectedApi.auth2 ? (detailBig ? 425 : 423) : (detailBig ? 383 : 381), marginLeft: -197, marginTop: selectedApi.name === "OpenWeather" ? (detailBig ? -222.5 : -221.5) : selectedApi.name === "Paystack" ? (detailBig ? -201.5 : -195.5) : selectedApi.auth2 ? (detailBig ? -212.5 : -211.5) : (detailBig ? -191.5 : -190.5), zIndex: 80, backgroundColor: isDark ? "#0f0f0f" : "#ffffff", borderRadius: 12, boxShadow: isDark ? "0 24px 60px rgba(0,0,0,0.6)" : "0 24px 60px rgba(0,0,0,0.18)", border: isDark ? "0.8px solid #2d2d2d" : "0.8px solid #f2f2f2", boxSizing: "border-box" }}
          >
            ...
            <motion.div variants={cardItemVariants} style={{ position: "absolute", left: 12, top: detailBig ? 44 : 40, display: "flex", alignItems: "center", gap: 4, borderRadius: 6, backgroundColor: selectedApi.name === "OpenAI" ? (isDark ? "#015145" : "#C1FFF6") : selectedApi.name === "Anthropic" ? (isDark ? "#202020" : "#181818") : selectedApi.name === "Replicate" ? (isDark ? "#4F0200" : "#EA0101") : selectedApi.name === "OpenWeather" ? (isDark ? "#EB563E" : "#EB563E") : (isDark ? "#000053" : "#e4e4ff"), padding: 4 }}>
              ...
              <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: 12, fontWeight: 500, letterSpacing: "-0.6px", color: selectedApi.name === "OpenAI" ? (isDark ? "#00FFD9" : "#00594B") : selectedApi.name === "Anthropic" ? "#FFFFFF" : selectedApi.name === "Replicate" ? (isDark ? "#FFD5D4" : "#FFE2E1") : selectedApi.name === "OpenWeather" ? "#FFFFFF" : (isDark ? "#C4C4FF" : "#5a5aff") }}>{selectedApi.name} Starter request</span>
            </motion.div>
            <div style={{ position: "absolute", left: 12, top: 76, width: 370, height: selectedApi.name === "OpenWeather" ? 357 : selectedApi.name === "Paystack" ? (detailBig ? 315 : 303) : selectedApi.auth2 ? (detailBig ? 337 : 331) : (detailBig ? 295 : 289), borderRadius: 10, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#0F0F0F" : "#ffffff", boxSizing: "border-box", overflow: "hidden" }}>
            <motion.div initial="hidden" animate="visible" exit="exit" variants={cardContentVariants} style={{ position: "absolute", left: 0, top: 0, right: 0, bottom: 0 }}>
              <motion.span variants={cardItemVariants} style={{ position: "absolute", left: 12, top: 12, fontFamily: "Geist, var(--font-geist-sans)", fontSize: 16, fontWeight: 500, letterSpacing: "-0.8px", color: isDark ? "#FFFFFF" : "#585858" }}>Test an endpoint</motion.span>
              <motion.span variants={cardItemVariants} style={{ position: "absolute", left: 12, top: 37, right: 12, fontFamily: "Geist, var(--font-geist-sans)", fontSize: 14, letterSpacing: "-0.48px", color: isDark ? "#F7F7F7" : "#939393", lineHeight: "20px" }}>{selectedApi.desc}</motion.span>
              {selectedApi.keyDesc ? (
                <motion.div variants={cardItemVariants} style={{ position: "absolute", left: 12, top: 73, width: 346, height: (selectedApi.name === "OpenWeather" || selectedApi.name === "Paystack") ? 94 : (detailBig ? 74 : 72), borderRadius: 8, outline: isDark ? "0.6px solid #844900" : "0.6px solid #FFDAAD", backgroundColor: isDark ? "#381E00" : "#FFFCFA" }}>
                  <div style={{ position: "absolute", left: 8, top: 8, display: "flex", alignItems: "center", gap: 4 }}>
                    ...
                    <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, fontWeight: 500, letterSpacing: detailBig ? "-0.7px" : "-0.6px", color: isDark ? "#FEC683" : "#DC7800" }}>Need a key</span>
                  </div>
                  {selectedApi.getAKeyUrl ? (
                    <a href={selectedApi.getAKeyUrl} target="_blank" rel="noopener noreferrer" style={{ position: "absolute", right: 8, top: 8, fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, fontWeight: 500, letterSpacing: detailBig ? "-0.7px" : "-0.6px", color: isDark ? "#FF9A20" : detailBig ? "#69380A" : "#693900", cursor: "pointer", textDecoration: "none" }}>Get a key</a>
                  ) : (
                    <span style={{ position: "absolute", right: 8, top: 8, fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, fontWeight: 500, letterSpacing: detailBig ? "-0.7px" : "-0.6px", color: isDark ? "#FF9A20" : detailBig ? "#69380A" : "#693900", cursor: "pointer" }}>Get a key</span>
                  )}
                  <div style={{ position: "absolute", left: 4, top: detailBig ? 34 : 32, width: 338, height: selectedApi.name === "OpenWeather" ? 56 : selectedApi.name === "Paystack" ? 56 : (detailBig ? 36 : 36), borderRadius: 6, backgroundColor: isDark ? "#693800" : "#FFF2E0" }}>
                    <span style={{ position: "absolute", left: 8, top: 8, right: 8, fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, lineHeight: "20px", letterSpacing: detailBig ? "-0.56px" : "-0.48px", color: isDark ? "#FFBF73" : "#A85C00", overflow: "hidden", whiteSpace: (selectedApi.name === "OpenWeather" || selectedApi.name === "Paystack") ? "normal" : "nowrap", textOverflow: "ellipsis" }}>{selectedApi.keyDesc}</span>
                  </div>
                </motion.div>
              ) : (
              ...
              <motion.div variants={cardItemVariants} style={{ position: "absolute", left: 12, top: (selectedApi.name === "OpenWeather" || selectedApi.name === "Paystack") ? 175 : (detailBig ? 155 : 161), width: 346, height: selectedApi.name === "OpenWeather" ? 170 : selectedApi.auth2 ? 170 : (detailBig ? 128 : 116), borderRadius: 8, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#0F0F0F" : "#ffffff", boxSizing: "border-box" }}>
                <div style={{ position: "absolute", left: 8, top: detailBig ? 10 : 8, display: "flex", alignItems: "center", gap: 4, borderRadius: 6, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#0F0F0F" : "#FCFCFC", padding: "6px 6px", height: detailBig ? 30 : 28, boxSizing: "border-box" }}>
                  <span style={{ width: 12, height: 12, borderRadius: "50%", backgroundColor: "#008000", flexShrink: 0 }} />
                  <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, letterSpacing: detailBig ? "-0.56px" : "-0.48px", color: isDark ? "#5A5A5A" : "#595959" }}>GET</span>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M4 6l6 6 6-6" stroke={isDark ? "#9e9e9e" : "#595959"} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <div style={{ position: "absolute", left: detailBig ? 89 : 86, top: 8, width: detailBig ? 249 : 252, height: detailBig ? 34 : 28, borderRadius: 6, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#212121" : "#ffffff", boxSizing: "border-box", display: "flex", alignItems: "center", padding: 8 }}>
                  <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, letterSpacing: detailBig ? "-0.56px" : "-0.48px", color: isDark ? "#F7F7F7" : "#5E5E5E", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{selectedApi.url}</span>
                </div>
                {selectedApi.auth ? (
                  <div style={{ position: "absolute", left: 8, top: detailBig ? 50 : 44, width: 330, height: detailBig ? 34 : 28, borderRadius: 6, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#212121" : "#ffffff", boxSizing: "border-box", display: "flex", alignItems: "center", padding: 8 }}>
                    <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, letterSpacing: detailBig ? "-0.56px" : "-0.48px", color: isDark ? "#D1D1D1" : "#9e9e9e", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{selectedApi.auth}</span>
                  </div>
                ) : null}
                {selectedApi.auth2 ? (
                  <div style={{ position: "absolute", left: 8, top: detailBig ? 92 : 84, width: 330, height: detailBig ? 34 : 28, borderRadius: 6, border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", backgroundColor: isDark ? "#212121" : "#ffffff", boxSizing: "border-box", display: "flex", alignItems: "center", padding: 8 }}>
                    <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: detailBig ? 14 : 12, letterSpacing: detailBig ? "-0.56px" : "-0.48px", color: isDark ? "#5E5E5E" : "#9e9e9e", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{selectedApi.auth2}</span>
                  </div>
                ) : null}
                {selectedApi.authNote ? (
                  <div style={{ position: "absolute", left: 16, right: 16, top: detailBig ? 88 : 80, height: detailBig ? 34 : 28, borderRadius: 6, display: "flex", alignItems: "center" }}>
                    <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: 12, letterSpacing: "-0.48px", color: isDark ? "#F7F7F7" : "#5E5E5E" }}>{selectedApi.authNote}</span>
                  </div>
                ) : null}
                <RippleButton style={{ position: "absolute", left: 8, top: selectedApi.authNote ? (detailBig ? 134 : 124) : selectedApi.auth2 ? (detailBig ? 134 : 124) : selectedApi.auth ? (detailBig ? 92 : 80) : (detailBig ? 50 : 44), display: "flex", alignItems: "center", gap: 4, borderRadius: 6, backgroundColor: "#5A5AFF", padding: "6px 6px", height: 28, boxSizing: "border-box" }}>
...
function UrlInputBar({  compact,  activeTheme,  method,  setMethod,  url,  setUrl,  loading,  onSend,}: {  compact?: boolean;  activeTheme: string;
 ...
 description2: string }> = {    GET: {      title: "/api/users/1",      status: "200",      rows: [        ["\u201Cid\u201D", ":", "1", ","],        ["\u201Cname\u201D", ":", "\u201CAda lovelace\u201D", ","],        ["\u201Cemail\u201D", ":", "\u201Cada@x.com\u201D", ","],      ],      jsonLabel: false,      label: "GET",      description: "Retrieve data from a resource without changing anything.",      description2: "",    },    POST: {      title: "POST",      status: "201 Created",      rows: [        ["\u201Cid\u201D", ":", "2", ","],        ["\u201Cname\u201D", ":", "\u201CGrace Hopper\u201D", ","],        ["\u201Cemail\u201D", ":", "\u201Cgrace@x.com\u201D", ","],      ],      jsonLabel: true,      label: "POST",      description: "Create a new resource with the data",      description2: "you send.",    },    PUT: {      title: "PUT",      status: "200 OK",      rows: [        ["\u201Cid\u201D", ":", "1", ","],        ["\u201Cname\u201D", ":", "\u201CAda Lovelace\u201D", ","],        ["\u201Cemail\u201D", ":", "\u201Cada@newmail.com\u201D", ","],      ],      jsonLabel: true,      label: "PUT",      description: "Replace a resource completely with new data.",      description2: "",    },    PATCH: {      title: "PATCH",      status: "200 OK",      rows: [        ["\u201Cid\u201D", ":", "1", ","],        ["\u201Cemail\u201D", ":", "\u201Cada@newmail.com\u201D", ","],      ],      jsonLabel: true,      label: "PATCH",      description: "Update part of a resource without replacing the rest.",      description2: "",    },    DELETE: {      title: "DELETE",      status: "204 No Content",      rows: [],      jsonLabel: true,      label: "DELETE",      description: "Remove a resource permanently.",      description2: "",    },  };
  ...
  return (    <div      className={`relative flex ${compact ? "w-[393px]" : "w-[467px]"} items-center gap-4 rounded-lg p-3`}      style={{                border: isDark ? "0.8px solid #2a2a2a" : "0.8px solid #f2f2f2",
        borderRadius: 8,
        backgroundColor: isDark ? "#121212" : "#ffffff",
        transition: "background-color 0.4s ease, border-color 0.4s ease",      }}    >      <div className="relative">        <button          onClick={() => { setShowDropdown(!showDropdown);
 ...
 setUrl(e.target.value);
 }}        className={`flex-1 bg-transparent text-sm tracking-[-0.04em] outline-none ${isDark ? "text-[#d1d1d1] placeholder:text-[#d1d1d1]" : "text-[#9e9e9e] placeholder:text-[#9e9e9e]"}`}        style={{ fontFamily: "Geist, var(--font-geist-sans)" }}      />            <RippleButton
        onClick={() => onSend()}
        disabled={loading}
