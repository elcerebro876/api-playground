...
function BrowseApiSection({ compact, w1440, isDark, onBack }: { compact?: boolean; w1440?: boolean; isDark: boolean; onBack: () => void }) {
  ...
  const apis: { name: string; desc: string; color: string; letter: string; category: string; logo?: string; logoColor?: string; keyDesc?: string; tag?: string; keyLabel?: string; cardBg?: string; panelBg?: string; panelBorder?: string; labelColor?: string; height?: number; url?: string; auth?: string; auth2?: string; authNote?: string; getAKeyUrl?: string }[] = [
          ...
          <div key={api.name} onMouseEnter={() => setHoveredApi(api.name)} onMouseLeave={() => setHoveredApi(null)} style={{ borderRadius: 4, backgroundColor: isDark ? (hoveredApi === api.name ? "#242424" : "#141414") : (api.cardBg ?? (hoveredApi === api.name ? "#fcfcfc" : "#ffffff")), border: isDark ? "0.8px solid #312F2F" : "0.8px solid #f2f2f2", padding: 12, display: "flex", flexDirection: "column", ...((compact || w1440) ? {} : { height: api.height ?? 205 }), transition: "background-color 0.15s ease", cursor: "pointer" }} onClick={() => setSelectedApi(api)}>
            ...
            {api.keyLabel ? (
              ...
              <div style={{ borderRadius: 4, backgroundColor: isDark ? "#2E1800" : (api.panelBg ?? "#fff4e8"), border: isDark ? "1px solid #934E00" : (api.panelBorder ?? "0.8px solid #FFE8CC"), padding: 8, display: "flex", flexDirection: "column", gap: 4, marginTop: (compact || w1440) ? 8 : "auto" }}>
                ...
                {api.keyDesc ? (() => {
                  const m = api.keyDesc.match(/(platform\.openai\.com|openweathermap\.org|console\.anthropic\.com)/);
                  if (!m) return <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 12 : 14, letterSpacing: compact || w1440 ? "-0.48px" : "-0.56px", color: "#DC7800", lineHeight: "20px" }}>{api.keyDesc}</span>;
                  const url = m[0], before = api.keyDesc.slice(0, m.index!), after = api.keyDesc.slice(m.index! + url.length);
                  return <span style={{ fontFamily: "Geist, var(--font-geist-sans)", fontSize: compact || w1440 ? 12 : 14, letterSpacing: compact || w1440 ? "-0.48px" : "-0.56px", color: "#DC7800", lineHeight: "20px" }}>{before}<a href={`https://${url}`} target="_blank" rel="noopener noreferrer" style={{ color: "#DC7800", textDecoration: "none" }} onMouseEnter={e => { e.currentTarget.style.textDecoration = "underline"; }} onMouseLeave={e => { e.currentTarget.style.textDecoration = "none"; }}>{url}</a>{after}</span>;
                })() : null}
              </div>
            )}
          </div>
